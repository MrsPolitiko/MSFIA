...
class ChatClient(QMainWindow):
    message_signal = pyqtSignal(str, bool, bool)  # message, is_system, is_my_message
    ...
    def __init__(self, host='127.0.0.1', port=5555):
        ...
        self.sound_signal.connect(self.play_notification_sound)
    ...
    def init_audio(self):
        # Получаем абсолютный путь к notification.wav относительно текущего файла
            ...
            try:
                ...
                if message:
                    ...
                    if message.startswith("[Сервер]"):
                        self.message_signal.emit(message, True, False)
                    else:
                        self.message_signal.emit(message, False, False)
                    self.sound_signal.emit()
            except Exception as e:
                self.message_signal.emit(f"[!] Ошибка соединения: {str(e)}", True, False)
    ...
    def send_message(self):
        ...
        if message.lower() == 'exit':
            ...
            try:
                ...
                self.message_signal.emit(message, False, True)
                self.message_input.clear()
            except Exception as e:
                self.message_signal.emit(f"[!] Ошибка отправки: {str(e)}", True, False)
